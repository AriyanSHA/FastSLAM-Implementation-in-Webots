...
def eval_sensor_model(measurements, particles):
    ...
    eps = np.finfo(float).eps   # numerical safety constant
    I = np.eye(2)
    # update landmarks and calculate weight for each particle
    # for each particle k
    for particle in particles:

        # w^[k] = 1 (or current weight)
        w = particle['weight']

        # for each observed landmark i
        for j, lm_id in enumerate(ids):

            # z_{t,i} in world coordinates
            z = np.array([
                x_dists[j] + particle['x'],
                y_dists[j] + particle['y']
            ])

            lm = particle['landmarks'][lm_id]

            # if landmark not observed before → initialize
            if not lm['observed']:
                lm['mu'] = z.tolist()
                lm['sigma'] = R_t.copy()
                lm['observed'] = True
            ...
            z_hat = mu                      # C_t = I
            Q = Sigma + R_t                 # innovation covariance
            K = Sigma @ np.linalg.inv(Q)    # Kalman gain
            ...
            likelihood = (
                1.0 / (2.0 * np.pi * np.sqrt(detQ))
                * np.exp(-0.5 * inno.T @ Q_inv @ inno)
            )
...
def resample_particles(particles):
    ...
    # Cumulative sum (used to pick particles)
    cdf = np.cumsum(weights)

    # SUS: one random start, evenly spaced picks
    ...
    for m in range(N):
        ...
        # Copy particle so maps are not shared
        p_new = copy.deepcopy(particles[i])
        p_new['weight'] = 1.0 / N
        new_pa.append(p_new)
